#!/bin/bash
# Install the Claude tasks skill

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SKILL_DIR="$HOME/.claude/skills/tasks"

echo "Installing Claude tasks skill..."
...
if [ -d "$SKILL_DIR" ] || [ -L "$SKILL_DIR" ]; then
    echo "Removing existing tasks skill..."
    rm -rf "$SKILL_DIR"
fi
...
if [ "$1" = "--copy" ]; then
    ...
    cp -r "$SCRIPT_DIR/skills/tasks" "$SKILL_DIR"
else
    echo "Symlinking skill to $SKILL_DIR..."
    ln -s "$SCRIPT_DIR/skills/tasks" "$SKILL_DIR"
fi
